...
const viewTaskHandler = (store: EnhancedStore, router: HistoryRouter) => (
  event: Event
) => {
  const months = [
    "January",
  ...
  if (tableCaption) {
    ...
    [year, month] = (event.target as HTMLDivDateElement).date
      .split(" ")[0]
      .split("-");
    numberOfMonth = month;
  }
  ...
  if (tableCaption && state.length > 0) {
    ...
    taskListRender(
      state.filter(
        (el: Task) => el.date.match(/[0-9]{4}-[0-9]{2}-[0-9]{2}/gm)[0] === date
      )
    );
  } else {
...
export function createViewTaskHandler(
  ...
  return viewTaskHandler(store, router);
}
